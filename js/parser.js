...
Parser.SPELL_ATTACK_TYPE_TO_FULL["R"] = "遠端";
...
Parser.monCrToFull = function (cr, {xp = null, isMythic = false} = {}) {
	...
	if (typeof cr === "string") {
		...
		if (cr.lair) stack.push(`當遭遇于巢穴時 ${Parser.monCrToFull(cr.lair)}`);
...
Parser.CAT_ID_TO_FULL[Parser.CAT_ID_SHIP_UPGRADE] = "船隻升級";
...
Parser.CAT_ID_TO_FULL[Parser.CAT_ID_ALCHEMICAL_FORMULA] = "鍊金師公式";
...
Parser.CAT_ID_TO_FULL[Parser.CAT_ID_CHAR_CREATION_OPTIONS] = "角色建立選項";
...
Parser.ATK_TYPE_TO_FULL["RW"] = "遠端武器攻擊";
...
Parser.SOURCE_JSON_TO_FULL[SRC_HotDQ] = "龍后的寶山";
...
Parser.SOURCE_JSON_TO_FULL[SRC_SADS] = "藍寶石週年紀念骰套組";
...
Parser.SOURCE_JSON_TO_ABV[SRC_WBtW] = SRC_WBtW;
...
Parser.SOURCE_JSON_TO_ABV[SRC_BGG] = "BGG";
...
Parser.SOURCE_JSON_TO_DATE[SRC_BGG] = "2023-08-15";
...
Parser.SKILL_JSON_TO_FULL = {
	"體操": [
		"你的敏捷（體操）檢定涵蓋了你在各種棘手情況下站穩的企圖，例如在冰面上奔跑、在拉緊的繩索上保持平衡、或在劇烈搖晃的甲板上維持直立。DM也可能會要求一次敏捷（體操）檢定以決定你能否做出體操體操，包括前翻、側翻、空翻、后翻等等。",
	],
	"馴獸": [
	...
	"運動": [
		"你的力量（運動）檢定涵蓋了各種當你在攀爬、跳躍、或游泳時會遭遇的困難情況。例子包括以下行動：",
		{
			"type": "list",
			"items": [
				"你嘗試攀爬一座陡峭或光滑的峭壁、在攀登墻壁時避開危險、或在有東西想把你擊落的情況下抓緊壁面。",
				"你嘗試跳出一段超遠的距離、或在跳躍途中展現一段特技動作。",
				"你拚命嘗試在兇險激流、風暴浪濤、或長滿層層海草的水域中游泳或維持漂浮。或者另一個生物試著將你推或拉入水中、或做出其他任何影響你游泳的行動。",
			],
		},
	],
	"欺瞞": [
		"你的魅力（欺瞞）檢定決定你是否能可信地隱瞞真相、無論是通過口頭言語或你的行動。從模棱兩可地誤導某人到撒下彌天大謊，欺瞞可以涵蓋幾乎所有行為。典型的情況包括嘗試用話術影響守衛、欺騙商人、從賭局贏取金錢、通過易容冒充某人、用虛妄的保證緩和某人的懷疑、或者在撒大謊時維持撲克臉等等。",
	],
	"歷史": [
	...
	"調查": [
		"當你四處查探線索並基於這些線索進行推理時，你進行一次智力（調查）檢定。你可能會因此推斷出某個隱藏物體的位置、從傷口的外觀判斷它是什麼武器造成的、或找出某個隧道中可能導致坍方的結構性弱點。爲了尋找隱藏的知識片段而鉆研古卷也可能會需要一次智力（調查）檢定。",
	],
	"醫療": [
	...
	"察覺": [
		"你的感知（察覺）檢定讓你能夠通過看、聽、或其他方式來發現某些東西的存在。它代表著你對周圍環境的總體意識以及你感官的敏銳度。", "舉例來說，你可以會嘗試聆聽門后的對話、在敞開的窗戶下竊聽、或聽見在森林中悄聲移動的怪物。或者，你可能會試著看見被遮蔽或容易看走眼的東西，無論它們是在前路埋伏的獸人、躲在暗巷里的混混、還是從緊閉的暗門門鏠中透出的燭光。",
	],
	"表演": [
	...
	"巧手": [
		"每當你嘗試表演晃眼花招或巧手，像是把某個東西放在他人身上或將一件東西藏在自已身上，進行一次敏捷（巧手）檢定。DM可能也會要求你進行敏捷（巧手）檢定以決定你是否能從他人的錢包中偷出錢幣、或從他人的口袋摸出某個東西。",
	],
	"隱匿": [
		"當你嘗試隱藏自己以躲避敵人、從守衛身邊溜過去、不被注意的潛逃、或無聲無息地偷偷接近某人時，進行一次敏捷（隱匿）檢定。",
	],
	"生存": [
		"DM可能會要求你進行一次感知（生存）檢定以追尋軌跡、狩獵野味、帶領你的隊伍穿越冰原、辨識梟頭熊生活于附近的徵兆、預測天氣、或者避開流沙以及其他自然危險。",
	],
};

Parser.SENSE_JSON_TO_FULL = {
	"盲視": [
		"具有盲視的生物即使不依賴視覺也可以感知其週遭特定半徑範圍內的環境。沒有眼睛的生物（像是泥怪）、以及具有回聲定位或高敏感官的生物（像是蝙蝠和真龍）都具有這種感官。",
	],
	"黑暗視覺": [
	...
	"真實視覺": [
		"具有真實視覺的生物在特定半徑範圍內，可以看透普通或魔法黑暗、看見隱形的生物和物體、自動偵測出視覺幻象併成功通過對抗它們的豁免檢定、並看穿變形者或被魔法變形的生物的原始型態。此外，這些生物也可以看見位於乙太位面的事物。",
	],
};
