...
class ClassesPage extends BaseComponent {
	...
	_render_renderSidebar () {
		...
		if (cls.multiclassing) {
			...
			if (mc.proficienciesGained) {
				$ptMcProfsIntro = $(`<div ${mc.requirements || mc.requirementsSpecial ? `class="cls-side__mc-prof-intro--requirements"` : ""}>當你不是以起始等級獲得新職業的等級，你只會獲得該職業一部分的起始熟練專案。</div>`);
	...
	_render_renderSubclassPrimaryControls ($wrp) {
		...
		this._listSubclass.on("updated", () => {
			...
			if (this._listSubclass.visibleItems.length) {
				...
				$dispCount.html(cntNotShown ? `<i class="clickable" title="調整篩選器以檢視更多。">(${cntNotShown}個條目未顯示)</i>` : "").click(() => this._doSelectAllSubclasses());
			} else if (this._listSubclass.items.length > 1) {
				$dispCount.html(`<i class="clickable" title="調整篩選器以檢視更多。">(${this._listSubclass.items.length - 1}個子職未顯示)</i>`).click(() => this._doSelectAllSubclasses());
			} else $dispCount.html("");
		});
	...
	async _render_pInitSubclassControls ($wrp) {
		...
		const filterSets = [
			{name: "檢視官方", subHashes: [], isClearSources: false},
			{name: "檢視最新", subHashes: [], isClearSources: true},
			{name: "檢視全部", subHashes: ["flstmiscellaneous:reprinted=0"], isClearSources: true},
		];
	...
	static _render_$getTrNoContent () {
		return $(`<tr class="cls-main__msg-no-content"><td colspan="6">切換任一按鈕來檢視職業和子職資訊</td></tr>`);
	}
