...
class InitiativeTracker {
	static make$Tracker (board, state) {
		...
		const $btnAdd = $(`<button class="btn btn-primary btn-xs dm-init-lockable" title="新增玩家"><span class="glyphicon glyphicon-plus"/></button>`).appendTo($wrpAdd);
		const $btnAddMonster = $(`<button class="btn btn-success btn-xs dm-init-lockable mr-2" title="新增怪物"><span class="glyphicon glyphicon-print"/></button>`).appendTo($wrpAdd);
		...
		$(`<button class="btn btn-default btn-xs mr-2"><span class="glyphicon glyphicon-cog"></span></button>`)
			...
			.click(() => {
				...
				const $cbStats = UiUtil.$getAddModalRowCb($modalInner, "新增額外欄位", cfg, "statsAddColumns");
		...
		const $btnLoad = $(`<button title="從怪物圖鑑匯入遭遇" class="btn btn-success btn-xs dm-init-lockable"><span class="glyphicon glyphicon-upload"/></button>`).appendTo($wrpLoadReset)
			.click((evt) => {
		...
		$btnAddMonster.on("click", () => {
			...
			const $wrpCount = $(`
				<div class="ui-search__ipt-search-sub-wrp" style="padding-right: 0;">
					<div style="margin-right: 7px;">新增</div>
		...
		async function pMakeRow (opts) {
			...
			$(`<button class="btn btn-warning btn-xs dm-init-row-btn dm-init-row-btn-flag" title="新增異常狀態" tabindex="-1"><span class="glyphicon glyphicon-flag"/></button>`)
				.appendTo($wrpConds)
				.on("click", () => {
					...
					const $btnAdd = $(`<button class="btn btn-primary">設定狀態</button>`)
						.click(() => {
