...
const ListUtil = {
	...
	initList (listOpts) {
		...
		if (ListUtil._firstInit) {
			...
			$headDesc.html(`${$headDesc.html()}按下J/K以巡覽列${ListUtil._isPreviewable ? `，按下M以擴充套件顯示` : ""}。`);
	...
	async _pDoSendSublistToFoundry () {
		...
		JqueryUtil.doToast(`嘗試發送 ${len} 條專案到 Foundry 之中。`);
	},
	...
	showTable (title, dataList, colTransforms, filter, sorter) {
		...
		const $btnCopy = $(`<button class="btn btn-primary">複製 CSV 到剪貼簿</button>`).click(async () => {
