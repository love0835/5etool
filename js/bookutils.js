...
class BookUtil {
	...
	static _showBookContent_renderNavButtons ({isTop, ixChapter, bookId, data}) {
		...
		if (isTop) {
			...
			const $btnEntireBook = $(`<a href="${href}" class="btn btn-xs btn-default no-print ${~BookUtil.curRender.chapter ? "" : "active"}" title="警告：速度較慢">檢視整本${this._getTypeTitleInCn(BookUtil.contentType)}</a>`);
	...
	static _showSearchBox (indexData, bookId, isPageMode) {
		...
		const $srch = $(`<input class="form-control" placeholder="${isPageMode ? "跳轉到頁碼..." : "搜尋文字..."}">`)
			.on("keydown", (e) => {
				e.stopPropagation();
		...
		return $$`<div class="contents-item" data-bookid="${UrlUtil.encodeForHash(book.id)}">
			<div class="bk__contents-header">
				...
				<div class="flex-v-center">
					<a href="${this._getHrefShowAll(book.id)}" class="bk__contents_show_all px-2 py-1p flex-v-center lst__wrp-cells lst__row-inner" title="檢視整本${this._getTypeTitleInCn(BookUtil.contentType)} （警告：速度較慢）">
