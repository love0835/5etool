	...
	static _getDeityBody (deity, reprintIndex) {
		...
		if (deity.entries) {
			renderer.recursiveRender(
				{
					entries: [
						...deity.customExtensionOf ? [`{@note 此神祇是{@deity ${deity.customExtensionOf}}加上來自於 <i title="${Parser.sourceJsonToFull(deity.source).escapeQuotes()}">${Parser.sourceJsonToAbv(deity.source)}</i> 的額外資訊的擴充套件。}`] : [],
						...deity.entries,
					],
				},
				renderStack,
		...
		return `
			${reprintIndex ? `
				<tr><td colspan="6">
				<i class="text-muted">
				${reprintIndex === 1 ? `此神祇是重印版本。` : ""} 以下版本被印于較舊的出版物中 (${Parser.sourceJsonToFull(deity.source)}${Renderer.utils.isDisplayPage(deity.page) ? `-第${deity.page}頁` : ""}).
				</i>
				</td></tr>
			` : ""}
