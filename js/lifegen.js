...
function getPersonDetails (opts) {
	...
	function addName (race, gender) {
		...
		if (nameTables[raceSlug]) {
			...
			out.unshift(`<i><b title="使用姍娜薩的萬事指南中的隨機姓名錶產生">名字：</b> ${resultFirst.result}${lastName ? ` ${lastName}` : ""}</i>`);
		}
	}
...
function sectParents () {
	...
	if (selRace === "Other") {
		$parents.html(concatSentences(`<b>種族：</b> 其他 ${fmtChoice(`${Parser.RaceToDisplay(race)}; 使用{@table Supplemental Tables; Race|XGE|補充種族}表產生`, true)}`, knowParentsStr, parentage));
	} else {
		$parents.html(concatSentences(`<b>種族：</b> ${Parser.RaceToDisplay(race)}${selRace === "Random" ? ` ${fmtChoice("使用{@table Supplemental Tables; Race|XGE|補充種族}表產生", true)}` : ""}`, knowParentsStr, parentage));
	}
...
function sectPersonalDecisions () {
	...
	$personal.append(`<b>我成爲了一名${myBg.name}，因為：</b> ${rollOnArray(myBg.reasons)}`);
}
...
function sectClassTraining () {
	...
	$clss.append(`<b>我成爲了一名${myClass.name}，因為：</b> ${rollOnArray(myClass.reasons)}`);
}
